  ...
  return (
    <>
      ...
      <section className="bg-white py-10">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="mb-6 flex items-start gap-3 rounded-2xl border border-gold/30 bg-gold/10 p-5 text-sm leading-relaxed text-navy/80">
            <Info className="mt-0.5 h-5 w-5 shrink-0 text-gold-dark" />
            <p><strong className="text-navy">A clear guide to our locations:</strong> Axis offices support consultation and coordination; learning centres host Axis programmes; partner facilities are approved venues; and educator hubs help us connect families with suitable educators. A listing does not necessarily mean Axis owns the facility.</p>
          </div>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
            {facilityTypes.map((facility) => (
              <div key={facility.type} className="rounded-xl border border-slate-200 p-5">
                <h2 className="font-semibold text-navy">{facility.type}</h2>
                <p className="mt-2 text-sm text-navy/70">{facility.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="bg-slate-50 py-10">
        <div className="mx-auto grid max-w-7xl gap-4 px-4 sm:px-6 lg:grid-cols-4 lg:px-8">
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <p className="text-xs uppercase tracking-[0.2em] text-slate-500">Total locations</p>
            <p className="mt-3 text-3xl font-bold text-navy">{stats.total}</p>
          </div>
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <p className="text-xs uppercase tracking-[0.2em] text-slate-500">Learning centres</p>
            <p className="mt-3 text-3xl font-bold text-navy">{stats.centres}</p>
          </div>
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <p className="text-xs uppercase tracking-[0.2em] text-slate-500">Partner facilities</p>
            <p className="mt-3 text-3xl font-bold text-navy">{stats.partner}</p>
          </div>
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <p className="text-xs uppercase tracking-[0.2em] text-slate-500">Head office</p>
            <p className="mt-3 text-3xl font-bold text-navy">{stats.headOffice}</p>
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="space-y-4 rounded-2xl border border-slate-200 bg-slate-50 p-4 shadow-sm">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                <label className="mb-2 block text-xs font-semibold uppercase tracking-[0.2em] text-navy/70">Location type</label>
                  ...
                  {LOCATION_TYPES.map((type) => (
                    <button
                      ...
                      className={`rounded-full px-4 py-2 text-sm font-medium transition-colors ${
                        selectedType === type ? 'bg-navy text-white' : 'bg-white text-navy hover:bg-slate-200'
                      }`}
                    >
                      {type === 'all' ? 'All' : type}
                    </button>
              ...
              <div>
                <label className="mb-2 block text-xs font-semibold uppercase tracking-[0.2em] text-navy/70">County</label>
                <select
                  value={selectedCounty}
                  onChange={(e) => setSelectedCounty(e.target.value)}
                  className="w-full rounded-lg border border-slate-300 bg-white px-4 py-2.5 focus:border-gold focus:outline-none focus:ring-2 focus:ring-gold/20"
                >
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          {loading ? (
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-12 text-center text-navy/70">
              <MapPin className="mx-auto mb-4 h-12 w-12 text-navy/30" />
            ...
            <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
              {filteredLocations.map((location, index) => (
                ...
                >
                  ...
                  <div className="space-y-4 p-5 text-sm text-navy/70">
                    ...
                    {location.programmes && location.programmes.length > 0 && (
                      <div>
                        <p className="mb-2 text-xs font-semibold uppercase tracking-[0.2em] text-navy/60">Programmes</p>
                        <div className="flex flex-wrap gap-2">
                          {location.programmes.slice(0, 4).map((programme) => (
                            <span key={programme} className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-medium text-navy">{programme}</span>
                          ))}
                        </div>
                      </div>
                    )}

                    <div className="border-t border-slate-200 pt-4 text-xs uppercase tracking-[0.15em] text-navy/60">
      ...
      <section className="bg-white py-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="max-w-3xl">
            ...
            <h2 className="text-3xl font-semibold text-navy">A national education network</h2>
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
            {[
              ...
              <div key={channel} className="rounded-xl border border-slate-200 bg-slate-50 p-5 text-sm font-medium text-navy">
                {channel}
              </div>
            ))}
          </div>
          ...
          <div className="mt-10">
            <p className="mb-4 text-xs font-semibold uppercase tracking-[0.2em] text-navy/60">
              All 47 counties
            </p>
            <div className="flex flex-wrap gap-2">
              {counties.map((county) => (
                <button
                  ...
                  className={`rounded-full border px-3 py-1.5 text-sm transition-colors ${
                    selectedCounty === county
                      ...
                      : 'border-slate-200 bg-white text-navy/70 hover:border-gold hover:text-navy'
                  }`}
                >
                  {county}
                </button>
      ...
      <section className="bg-navy py-14 text-white">
        <div className="mx-auto flex max-w-7xl flex-col items-start justify-between gap-6 px-4 sm:px-6 lg:flex-row lg:items-center lg:px-8">
          ...
          <Link
            to="/enroll"
            className="inline-flex shrink-0 items-center gap-2 rounded-lg bg-gold px-5 py-3 text-sm font-semibold text-navy transition-colors hover:bg-gold-light"
          >
