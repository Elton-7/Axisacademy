        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid gap-4 rounded-2xl border border-slate-200 bg-slate-50 p-4 shadow-sm md:grid-cols-3">
            <div className="rounded-xl bg-white p-4">
              <p className="text-xs uppercase tracking-[0.2em] text-slate-500">Partners</p>
            ...
            <div className="rounded-xl bg-white p-4">
              ...
              <p className="mt-3 text-lg font-semibold text-navy">Growing</p>
            </div>
          </div>
        </div>
      </section>
                ...
                >
                  ...
                  <div className="space-y-4 p-5">
                    <div className="flex items-center justify-between gap-3">
                      <span className="rounded-full bg-gold/15 px-2.5 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-navy">{partner.category}</span>
                    </div>

                    <div>
                      <h3 className="text-xl font-semibold text-navy">{partner.name}</h3>
                      {partner.description && <p className="mt-2 text-sm leading-relaxed text-navy/70 line-clamp-3">{partner.description}</p>}
                    </div>

                    {partner.focusAreas && partner.focusAreas.length > 0 && (
                      <div className="flex flex-wrap gap-2">
                        {partner.focusAreas.slice(0, 3).map((area) => (
                          <span key={area} className="rounded-full bg-slate-100 px-2.5 py-1 text-xs text-navy/70">{area}</span>
                        ))}
                      </div>
                    )}
                    ...
                    {(partner.website || partner.email || partner.phone) && (
                      <div className="space-y-2 border-t border-slate-200 pt-4">
                        ...
                        {partner.email && <p className="text-xs text-navy/60">{partner.email}</p>}
                        {partner.phone && <p className="text-xs text-navy/60">{partner.phone}</p>}
                      </div>
                    )}
                  </div>
                </motion.article>
