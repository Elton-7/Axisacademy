...
function SectionBlock({ section }: { section: ServiceSection }) {
  return (
    ...
    >
      <h2 className="text-2xl font-semibold text-ink sm:text-3xl">{section.heading}</h2>

      {section.body && (
        <p className="mt-4 max-w-3xl text-lg leading-relaxed text-ink-muted">{section.body}</p>
      )}
      ...
      {section.options && (
        <div className="mt-8 grid gap-4 md:grid-cols-2">
          {section.options.map((option) => (
            <div key={option.title} className="rounded-2xl border border-line bg-surface p-6 shadow-sm">
      ...
      {section.note && (
        <div className="mt-8 rounded-2xl border-l-4 border-gold-500 bg-gold-50 p-6">
          <p className="text-sm leading-relaxed text-ink-muted">{section.note}</p>
        </div>
      )}
    </motion.section>
  )
    ...
    <div className="pt-20">
      ...
      <section className="bg-navy-900 py-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          >
            <div className="mb-6 flex h-14 w-14 items-center justify-center rounded-2xl border border-gold-500/40 bg-surface/5">
      ...
      <section className="bg-surface-sunk py-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <h2 className="mb-10 text-center text-3xl font-semibold text-ink">Other Axis services</h2>
              ...
              return (
                ...
                >
                  <div className={`mb-4 flex h-12 w-12 items-center justify-center rounded-xl ${entry.accent}`}>
                    <EntryIcon className="h-6 w-6 text-ink" />
                  </div>
